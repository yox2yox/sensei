...
- **比喩は本文の中でも使い続けます**。`metaphor` で選んだたとえを宣言して終わりにせず、`scenes[].action` / `result` / `takeaway` の中で型名・クラス名と並べて言い直しましょう（例: 「`auth-validator` が呼ばれる」ではなく「**受付係**である `auth-validator` が呼ばれる」）。比喩語が本文から消えたら、それは「コードの説明」に縮退しているサインです。
